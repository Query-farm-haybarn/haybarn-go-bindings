...
The Haybarn static-library archives for `darwin-amd64` are fetched from a Haybarn engine
release, not committed by hand and not carried over from upstream DuckDB.

Populate them with:

    make fetch.static.libs PLATFORM=darwin-amd64 FILENAME=<asset-name>
