...
if [ "${current_branch}" != "haybarn" ] && [ "${current_branch}" != "main" ] && [[ "${current_branch}" != v1.* ]]; then
  echo "error: release must be run on the haybarn branch (or main / a v1.* branch)" >&2
  exit 1
fi
...
for dir in "${lib_dirs[@]}"; do
  lib_mods+=("github.com/Query-farm-haybarn/haybarn-go-bindings/${dir}")
  tag="${dir}/${version}"
  lib_tags+=("${tag}")
...
export GOPRIVATE="github.com/Query-farm-haybarn/haybarn-go-bindings" # bypass proxy/sumdb for these modules
